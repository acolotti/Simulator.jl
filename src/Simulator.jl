...
function simulate(f::Function, x0::AbstractArray{<:Real}, p::Vector{Any}, t::Real; solver=DE.TRBDF2, quiet::Bool=false, 
    ...
    if quiet
        for i = 1:s
            trajectories[i] = singleSimulation(f,x0[:,i],p,t,solver,terminal_state=terminal_state,terminal_threshold=terminal_threshold);
        end
    else
        for i = 1:s
...
function simulate(f::Function, x0::AbstractArray{<:Real}, P::AbstractMatrix{<:Real}, sd::AbstractArray{<:Real}, t::Real; controller=VS.transposeController, solver=DE.TRBDF2, 
    quiet::Bool=false, terminal_state::Union{Vector{T} where T<:Real, Nothing} = nothing, terminal_threshold::Real = 1e-6)

    return simulate(f,x0,[P,sd,controller],t,solver=solver,quiet=quiet,terminal_state=terminal_state,terminal_threshold=terminal_threshold);
end

function singleSimulation(f::Function, x0::AbstractArray{<:Real}, p::Vector{Any}, t::Real, solver; terminal_state::Union{Vector{T} where T<:Real, Nothing} = nothing, 
    terminal_threshold::Real = 1e-6)
    ...
    if isa(terminal_state,Nothing)
        sol = DE.solve(prob,solver(),reltol=tol,abstol=tol);
    else
        norm²(v) = sum(v.^2);
        ...
        sol = DE.solve(prob,solver(),callback=cb);
